...
sub required{
  my @reqs=( qw/SlurmHC::Nfs SlurmHC::SlurmHC::Nfs/ );
  return @reqs;
}
...
sub run{
  ...
  my $self;
  if ($_[0] || ref $_[0]) {
    $self = shift if $_[0]->can('isa') and  $_[0]->isa('SlurmHC::Schroot::Nfs');
  }
  ...
  foreach (@sessions) {
    ...
    my @mounts=`su $arg->{run_as} -c 'schroot -r -d $arg->{run_dir} -c $session -- df -t nfs | grep -v Available'`;
    foreach (@mounts){
      my @data = split /\s+/, $_;
      my $mountdir = $data[5];
      my $mountpoint = $data[0];
      my $syscall="su $arg->{run_as} -c 'schroot -d $arg->{run_dir} -r -p -c ".$session." -- ls -l $mountdir/ &>/dev/null || exit 1'";
      system($syscall);
      if($? != 0){
	push $results->{error},
	...
	push $results->{info},
	  (caller(0))[3].": all ok with $mountpoint, mounted on $mountdir in session $session.";
      }
    }
  }
...
=head1 NAME
    ...
    SlurmHC::Schroot::Nfs - Slurm healtcheck (sub)package for testing nfs mounts within chroot sessions

=head1 SYNOPSIS

    use SlurmHC qw( Schroot::Nfs );
